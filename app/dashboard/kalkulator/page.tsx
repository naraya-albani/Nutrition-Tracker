...
export default function KalkulatorPage() {
  ...
  const calculateNutrition = (e?: React.FormEvent) => {
    ...
    setProtein(Math.round((finalCalories * 0.20) / 4)); // 1g protein = 4 kalori
    setFat(Math.round((finalCalories * 0.30) / 9)); // 1g lemak = 9 kalori
    setCarbs(Math.round((finalCalories * 0.50) / 4)); // 1g karbo = 4 kalori
    ...
    <div className="min-h-screen bg-[#f7f8f4] font-sans flex flex-col items-center pb-20">
      ...
      <nav className="w-full bg-[#113c23] px-6 sm:px-10 py-4 flex items-center gap-10 sticky top-0 z-50">
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          <Link href="/kalkulator" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#174e2d] text-white font-medium shadow-inner border border-white/5">
      ...
      <main className="w-full max-w-[1000px] px-6 sm:px-10 py-10">
        ...
        <h1 className="text-[28px] font-bold text-gray-900 tracking-tight mb-8">Kalkulator gizi</h1>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">

          {/* Left Column - Form */}
          <form onSubmit={calculateNutrition} className="bg-white rounded-[24px] p-6 sm:p-8 shadow-sm border border-gray-100 h-fit">
            <h3 className="font-bold text-gray-900 text-[18px] mb-6">Data diri</h3>

            {/* Gender Toggle */}
            <div className="mb-5">
              <label className="block text-[13px] font-bold text-gray-600 mb-2">Jenis kelamin</label>
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={() => setGender('Perempuan')}
                  className={`flex-1 py-3 px-4 rounded-xl font-bold text-[14px] transition-all border ${gender === 'Perempuan' ? 'border-gray-800 text-gray-900 bg-white shadow-sm' : 'border-gray-200 text-gray-500 hover:border-gray-300'
                    }`}
                >
                  Perempuan
                </button>
                <button
                  ...
                  className={`flex-1 py-3 px-4 rounded-xl font-bold text-[14px] transition-all border ${gender === 'Laki-laki' ? 'border-gray-800 text-gray-900 bg-white shadow-sm' : 'border-gray-200 text-gray-500 hover:border-gray-300'
                    }`}
                >
                  Laki-laki
                </button>
            ...
            <div className="flex gap-4 mb-5">
              <div className="flex-1">
                <label className="block text-[13px] font-bold text-gray-600 mb-2">Usia</label>
                <input
                  type="number"
                  value={age}
              ...
              <div className="flex-1">
                <label className="block text-[13px] font-bold text-gray-600 mb-2">Tinggi badan (cm)</label>
                <input
                  type="number"
                  value={height}
            ...
            {/* Weight */}
            <div className="mb-5">
              <label className="block text-[13px] font-bold text-gray-600 mb-2">Berat badan (kg)</label>
              <input
                type="number"
                value={weight}
            ...
            {/* Activity Level */}
            <div className="mb-5">
              <label className="block text-[13px] font-bold text-gray-600 mb-2">Tingkat aktivitas</label>
              <div className="relative">
                ...
                >
                  ...
                  <option value="1.9">Sangat Aktif (Fisik berat tiap hari)</option>
                </select>
                <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-600 font-bold" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            ...
            {/* Goal */}
            <div className="mb-8">
              <label className="block text-[13px] font-bold text-gray-600 mb-2">Tujuan</label>
              <div className="relative">
                <select
                  value={goal}
          ...
          <div className="bg-[#24422e] rounded-[24px] p-6 sm:p-8 shadow-sm text-white h-fit">
            <h3 className="font-bold text-[#8ca896] text-[16px] mb-8">Hasil perhitungan</h3>

            <div className="flex flex-col items-center mb-8">
              <div className="text-[64px] font-bold leading-none tracking-tight mb-2">
                {calories.toLocaleString('id-ID')}
              </div>
              <p className="text-[#8ca896] font-medium text-[15px]">Kebutuhan kalori harian (kkal)</p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-8">
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col items-center justify-center">
                <span className="font-bold text-[22px] leading-tight mb-1">{protein} g</span>
                <span className="text-[#8ca896] text-[12px] font-medium">Protein</span>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col items-center justify-center">
                <span className="font-bold text-[22px] leading-tight mb-1">{fat} g</span>
                <span className="text-[#8ca896] text-[12px] font-medium">Lemak</span>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col items-center justify-center">
                <span className="font-bold text-[22px] leading-tight mb-1">{carbs} g</span>
                <span className="text-[#8ca896] text-[12px] font-medium">Karbohidrat</span>
              </div>
            </div>

            <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex justify-between items-center">
              <div>
                <div className="font-bold text-[20px]">BMI {bmi.toLocaleString('id-ID')}</div>
                <div className="text-[#8ca896] text-[13px] font-medium">Indeks massa tubuh</div>
              </div>
              <div className={`px-4 py-1.5 rounded-full text-[13px] font-bold ${bmiCategory === 'Normal' ? 'bg-[#42b26e]/20 text-[#42b26e]' :
                  bmiCategory === 'Kurus' ? 'bg-blue-500/20 text-blue-400' :
