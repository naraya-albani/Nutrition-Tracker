...
export default function ScanMakananPage() {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      const apiUrl =
        process.env.NEXT_PUBLIC_MODEL_URL ?? "http://localhost:8000";
      ...
      setDetectedFood({
        name: result.combined.name,
        emoji: result.combined.emoji,
        accuracy: result.combined.accuracy,
        calories: result.combined.calories,
        protein: result.combined.protein,
        fat: result.combined.fat,
        carbs: result.combined.carbs,
        satFat: result.combined.satFat,
        fiber: result.combined.fiber,
        sugar: result.combined.sugar,
        sodium: result.combined.sodium,
        portionSize: result.combined.portionSize,
      });
      setPortion(result.combined.portionSize);
      setShowResult(true); // hanya tampil kalau sukses
    } catch (err) {
      console.error("Prediksi gagal:", err);
      setPreviewUrl(null);
      alert(
        `Gagal menganalisis foto: ${err instanceof Error ? err.message : "Coba lagi"}`,
      );
    } finally {
      setIsScanning(false);
      // showResult TIDAK di-set di sini
    }
  };
  ...
  const handleReset = () => {
    ...
    setPreviewUrl(null);
    setPortion(300);
    setDetectedFood(null); // tambahkan ini
  };

  const portionFactor = detectedFood ? portion / detectedFood.portionSize : 0;
  const currentCalories = detectedFood
    ? Math.round(detectedFood.calories * portionFactor)
    : 0;
  const currentProtein = detectedFood
    ? Math.round(detectedFood.protein * portionFactor)
    : 0;
  const currentFat = detectedFood
    ? Math.round(detectedFood.fat * portionFactor)
    : 0;
  const currentCarbs = detectedFood
    ? Math.round(detectedFood.carbs * portionFactor)
    : 0;
  const currentSatFat = detectedFood
    ? (detectedFood.satFat * portionFactor).toFixed(1)
    : "0";
  const currentFiber = detectedFood
    ? (detectedFood.fiber * portionFactor).toFixed(1)
    : "0";
  const currentSugar = detectedFood
    ? (detectedFood.sugar * portionFactor).toFixed(1)
    : "0";
  const currentSodium = detectedFood
    ? Math.round(detectedFood.sodium * portionFactor)
    : 0;
  ...
  return (
    <div className="min-h-screen bg-[#f7f8f4] font-sans flex flex-col items-center pb-20">
      ...
      {/* Main Content */}
      <main className="w-full max-w-300 px-6 sm:px-10 py-10">
        {/* Page Header */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {isScanning ? (
              ...
              <div className="w-full aspect-4/3 bg-[#112d1b] rounded-3xl relative overflow-hidden shadow-inner flex flex-col items-center justify-center border-4 border-[#123e25]">
                ...
                <div
                  ...
                  style={{
                    backgroundImage:
                      "linear-gradient(#42b26e 1px, transparent 1px), linear-gradient(90deg, #42b26e 1px, transparent 1px)",
                    backgroundSize: "20px 20px",
                  }}
                />
                <div className="absolute top-6 left-6 w-8 h-8 border-t-2 border-l-2 border-[#42b26e]" />
                <div className="absolute top-6 right-6 w-8 h-8 border-t-2 border-r-2 border-[#42b26e]" />
                <div className="absolute bottom-6 left-6 w-8 h-8 border-b-2 border-l-2 border-[#42b26e]" />
                <div className="absolute bottom-6 right-6 w-8 h-8 border-b-2 border-r-2 border-[#42b26e]" />
                <div
                  className="absolute left-0 w-full h-0.75 bg-[#42b26e] shadow-[0_0_15px_3px_rgba(66,178,110,0.8)] z-10"
                  style={{ animation: "scanLaserLine 2s ease-in-out infinite" }}
                />
                <div className="relative z-20 flex flex-col items-center text-center -mt-10">
                  <span className="text-4xl mb-4 animate-bounce">
                    {detectedFood?.emoji ?? "🍽️"}
                  </span>
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="w-full bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-200">
                  ...
                  >
                    ...
                    <div className="absolute top-[20%] left-[35%] w-37.5 h-30 border-2 border-[#42b26e] rounded-xl bg-[#42b26e]/10 shadow-[0_0_10px_rgba(66,178,110,0.2)]">
                  ...
                  <div className="p-5 flex justify-between items-center">
                    <div>
                      <h4 className="font-bold text-gray-900 text-[15px] truncate max-w-50 sm:max-w-75">
                        {fileName}
                      </h4>
                    ...
                    <span className="bg-green-50 text-green-700 px-3 py-1.5 rounded-full text-[13px] font-bold">
                      Akurasi {detectedFood?.accuracy}%
                    </span>
                  </div>
                </div>
                ...
                {/* Adjust Portion */}
                <div>
                  <h3 className="font-bold text-gray-900 mb-4 text-[16px]">
                    Sesuaikan porsi
                  </h3>
                  ...
                  <p className="text-gray-400 text-[13px] font-medium">
                    AI mengestimasi 1 porsi ≈ {detectedFood?.portionSize}g
                    berdasarkan foto
                  </p>
                </div>
                ...
                <div className="flex gap-4 mt-2">
                  <button
                    onClick={handleReset}
                    className="flex-1 flex items-center justify-center gap-2 bg-white border border-gray-300 text-gray-700 font-bold py-3 px-4 rounded-xl hover:bg-gray-50 transition-colors shadow-sm"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-5 w-5"
              ...
              <div
                ...
                onDrop={handleDrop}
                className={`w-full aspect-4/3 rounded-3xl relative overflow-hidden flex flex-col items-center justify-center border-2 border-dashed transition-all cursor-pointer group ${
                  isDragging
          ...
          <div className="flex flex-col gap-6">
            {showResult ? (
              ...
              <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-gray-100 min-h-85">
                {/* Card Header */}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 text-[13px] font-medium mt-0.5">
                        1 porsi &middot; est. {detectedFood.portionSize}g
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="font-bold text-[#42b26e] text-[18px]">
                      {detectedFood.accuracy}%
                    </div>
                    <div className="text-gray-400 text-[12px] font-medium mt-0.5">
                      akurasi
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-3 mb-8">
                  <div className="bg-[#fcfaf7] border border-gray-100 rounded-2xl p-4 flex flex-col items-center justify-center shadow-sm">
                    <span className="text-[#c8812c] font-bold text-[24px] leading-tight mb-1">
                      {currentCalories}
                    </span>
                  ...
                  <div className="bg-[#fcfaf7] border border-gray-100 rounded-2xl p-4 flex flex-col items-center justify-center shadow-sm">
                    <span className="text-[#1e62a8] font-bold text-[24px] leading-tight mb-1">
                      {currentProtein}g
                    </span>
                    <span className="text-gray-400 text-[12px] font-semibold">
                      Protein
                  ...
                  <div className="bg-[#fcfaf7] border border-gray-100 rounded-2xl p-4 flex flex-col items-center justify-center shadow-sm">
                    <span className="text-[#e05252] font-bold text-[24px] leading-tight mb-1">
                      {currentFat}g
                    </span>
                    <span className="text-gray-400 text-[12px] font-semibold">
                      Lemak
                  ...
                  <div className="bg-[#fcfaf7] border border-gray-100 rounded-2xl p-4 flex flex-col items-center justify-center shadow-sm">
                    <span className="text-[#42b26e] font-bold text-[24px] leading-tight mb-1">
                      {currentCarbs}g
                    </span>
                    <span className="text-gray-400 text-[12px] font-semibold">
                      Karbo
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex items-center gap-4">
                    <span className="text-[14px] font-bold text-gray-700 w-24">
                      Lemak jenuh
                    </span>
                    ...
                    <span className="text-[14px] font-bold text-red-500 w-12 text-right">
                      {currentSatFat}g
                    </span>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[14px] font-bold text-gray-900 w-12 text-right">
                      {currentFiber}g
                    </span>
                  </div>
                  <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[14px] font-bold text-red-500 w-16 text-right">
                      {currentSodium}mg
                    </span>
                  </div>
                </div>
                ...
                {/* Recommendations */}
                <h4 className="font-bold text-[15px] text-gray-900 mb-4">
                  Rekomendasi
                </h4>
                <div className="space-y-4 mb-10">
                  {currentSodium > 600 && (
                    <div className="flex gap-4 p-4 rounded-2xl bg-red-50 border border-red-100">
                      ...
                      <div>
                        <h5 className="font-bold text-red-700 text-[14px] mb-1">
                          Natrium cukup tinggi
                        </h5>
                        <p className="text-red-600/80 text-[13px] leading-relaxed">
                          {currentSodium}mg = {Math.round(currentSodium / 20)}%
                          kebutuhan harian. Kurangi garam di makan malam.
                        </p>
                      </div>
                    </div>
                  )}
                  <div className="flex gap-4 p-4 rounded-2xl bg-green-50 border border-green-100">
                    ...
                    <div>
                      <h5 className="font-bold text-green-700 text-[14px] mb-1">
                        Protein mencukupi
                      </h5>
                      <p className="text-green-700/80 text-[13px] leading-relaxed">
                        {currentProtein}g sudah memenuhi kebutuhan gizi seimbang
                        harian Anda.
                      </p>
                    </div>
                  </div>
                  {parseFloat(currentFiber) < 3.0 && (
                    <div className="flex gap-4 p-4 rounded-2xl bg-orange-50 border border-orange-100">
                      ...
                      <div>
                        <h5 className="font-bold text-orange-700 text-[14px] mb-1">
                          Tambah sayuran
                        </h5>
                        <p className="text-orange-700/80 text-[13px] leading-relaxed">
                          Serat hanya {currentFiber}g. Tambahkan sayuran hijau
                          untuk melengkapi nutrisi harian.
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <div className="flex flex-col gap-3">
                  <button
                    onClick={() => alert("Berhasil ditambahkan ke log makan!")}
                    className="w-full flex items-center justify-center gap-2 bg-[#123e25] text-white hover:bg-[#1a5b36] font-bold py-3.5 px-6 rounded-xl transition-all shadow-sm"
                  >
                    <span className="text-xl leading-none">+</span> Tambahkan ke
                    log makan
                  </button>
                </div>
              </div>
              ...
              <>
                {/* Status Card */}
                <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 flex flex-col items-center justify-center min-h-85">
                  ...
                  <div className="relative w-full h-full flex flex-col items-center justify-center">
                    ...
                    <p className="text-gray-500 text-center max-w-70 font-medium leading-relaxed">
                      Ambil foto atau unggah gambar makanan untuk melihat
